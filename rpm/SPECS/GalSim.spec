...
scons PREFIX=%{_build_prefix} PYTHON=%{_build_prefix}/bin/python3 PYPREFIX=%{_build_prefix}/lib/python3.6/site-packages/ EXTRA_LIB_PATH=%{_build_prefix}/lib64 EXTRA_INCLUDE_PATH=%{_build_prefix}/include

%install
scons install PREFIX=%{buildroot}%{_prefix}
# Necessary for check build without BUILDROOT in binaries
sed -i "s|%{buildroot}||g" %{_build_prefix}/lib/python3.6/site-packages/galsim/meta_data.py
mkdir -p %{buildroot}%{_prefix}/lib/python3.6/site-packages
cp -r %{_build_prefix}/lib/python3.6/site-packages/galsim %{buildroot}%{_prefix}/lib/python3.6/site-packages
